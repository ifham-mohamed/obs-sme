...
**Phase 2 DoD (overall):** invoking the pipeline on a fresh `gazette.lk` URL ends with `m1_regulations` row at `status='extracted'`, all metadata fields populated. Status badges in 14_M1_1 ([Pipeline State Tracking](14_M1_1_Admin_Pipeline_State_Tracking.md)) flip from 🟡 to ✅ on this surface.
