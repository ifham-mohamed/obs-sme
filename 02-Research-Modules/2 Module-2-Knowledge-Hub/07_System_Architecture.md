...
- `/surveys` — survey mode selector / entry hub
- `/surveys/unified` — unified regulation-block survey
- `/surveys/module/1` — Module 1 awareness survey
- `/surveys/module/2` — Module 2 knowledge survey
- `/surveys/module/3` — Module 3 risk survey
- `/surveys/module/4` — Module 4 misinformation survey
- `/surveys/history` — current SME session history
...
- `/admin/survey/questions` — survey question bank management
- `/admin/survey/regulations` — survey regulation-sector mapping management
...
POST /survey-sessions/start
GET  /survey-sessions/my-history
GET  /survey-sessions/{session_id}
POST /survey-sessions/{session_id}/complete

GET  /surveys/unified/start?session_id={session_id}
POST /surveys/unified/answer?session_id={session_id}

GET  /surveys/module/{module_number}/questions?session_id={session_id}
POST /surveys/module/{module_number}/answer?session_id={session_id}
...
The survey service owns mode-aware question selection and in-session progression:

```python
def get_questions_for_sme(sme_profile, mode):
    """Return the sector-filtered question set for one SME and one mode."""

def get_next_question(session_id, current_question_id, answer_payload):
    """Persist the answer, evaluate the next step, and return the next question or completion."""

def _condition_met(condition, prior_answers, sector):
    """Evaluate trigger_condition JSON against prior answers and sector visibility."""
```

Expected behavior:

- `get_questions_for_sme(mode)` applies sector filtering via `regulation_sector_map`
- unified mode builds regulation blocks and applies the 15–20 total cap
- module modes apply the 8–10 question cap
- `get_next_question()` never branches across regulation blocks
- `_condition_met()` evaluates answer-based and sector-based trigger conditions
