...
**Implementation status (synced 2026-08-01):** Steps 41-47 have now run in Kaggle as an additive working experiment. The frozen `linearsvc_v6_primary` remains untouched and remains the production category classifier. The V7 branch produced a no-leak 1103-row training dataset, patched the Kaggle parser/trainer, completed smoke and 3-seed training, and was **not promotable**: the final run collapsed to majority-category behaviour and one-or-zero sector predictions. Everything after Step 47 remains blocked on weighted-loss recovery.
...
| 48     | Weighted-loss recovery + threshold selection on validation | ⬜ **next** — requires a credible diagnostic           |
| 49     | Three-seed recovery final                                  | ⛔ blocked on Step 48                                  |
