---
title: "Enigmatrix — Consolidated Final Report (Group Final Draft + Module 1 Individual Report)"
...
TF-IDF baselines and an XLM-RoBERTa encoder with LoRA adapters and a dual head, exported to ONNX for CPU inference.
...
- 4.Classify. The ONNX-exported XLM-R + LoRA model assigns one of eight change categories
and a multi-label set of affected sectors, with a confidence score. Predictions below the 0.55 threshold are routed to expert review; records already marked expert-verified are never overwritten. The record advances to classified.
...
> **What the diagram shows.** A state machine for a regulation record. From the start node, the *Scrapy spider (date-scoped, EN→SI→TA fallback)* produces state **ingested**; `extract_gazette` (classify_pdf then text / hybrid / scanned route) produces **extracted**; `preprocess_gazette` (clean, fastText LID, Wijesekara, metadata, chunk) produces **preprocessed**; `classify_gazette` (ONNX XLM-R + LoRA) produces **classified**; `dispatch_regulation_alerts` (sector-matched, idempotent) produces **alerted**, then the terminal state. Two guarded detours route into a **review** state: *metadata_confidence below threshold* from preprocessed, and *classifier_confidence < 0.55* from classified. The return edge is *expert verification (never overwritten)*, back to classified.
...
|---|---|---|---|---|---|---|---|---|
| Labelled dataset with established reliability |  |  | 800 rows; κ = 0.8715 / 0.8638 / 0.7235; 40 adjudicated disagreements |  |  | Achieved |  |  |
...
This dissertation documents the platform and, in detail, Module 1. Four Scrapy spiders collect gazettes, acts and bills. A per-page routing extraction chain built on PyMuPDF, pdfplumber, pypdfium2, Tesseract 5 and Surya OCR, with font-aware Wijesekara-to-Unicode conversion, handles a corpus that is part born-digital English and part scanned legacy-encoded Sinhala and Tamil. Preprocessing performs cleaning, fastText language identification, metadata and penalty extraction, chunking and sub-document splitting. A gold dataset of 800 regulation records was constructed by dual annotation followed by explicit adjudication, achieving a category Cohen's kappa of 0.8715, a mean sector kappa of 0.8638 and an SME-relevance kappa of 0.7235. TF-IDF baselines reach test macro-F1 of 0.4980 for logistic regression and 0.6167 for LinearSVC, quantifying the headroom to the 0.92 project gate. A single XLM-RoBERTa encoder with LoRA adapters and a dual head — single-label category by cross-entropy over eight categories, multi-label sector by binary cross-entropy over three sectors — was implemented, validated by a local CPU smoke test and trained on free GPU runtimes, then exported to ONNX for CPU inference. Downstream, propagation watchers, an idempotent sector-matched alert dispatcher and nightly lag analytics convert the platform into a research instrument that makes regulatory information diffusion an empirical quantity rather than an anecdote.
...
flowchart TD
    ...
    S3 --> S4["Stage 4 - Classify<br/>XLM-R + LoRA dual head via ONNX<br/>confidence gate 0.55"]
...
stateDiagram-v2
    ...
    classified --> review: classifier_confidence < 0.55
...
| Role | Extraction, preprocessing, measurement, TF-IDF baselines, CPU smoke test | XLM-R + LoRA training, slice evaluation, ONNX export, NLLB backfill |
| Runtime per seed | not applicable | [ADD GPU RUNTIME] minutes for 560 training rows |
...
| Model | Test macro-F1 | Test accuracy | Sector micro-F1 | Sector macro-F1 | Gate (≥ 0.92) |
|---|---|---|---|---|---|
| TF-IDF logistic regression | 0.4980 | — | — | — | Fail |
| TF-IDF LinearSVC | 0.6167 | — | — | — | Fail |
| XLM-R + LoRA (CPU smoke) | 0.0000 | — | — | — | Fail (by design) |
| XLM-R + LoRA (GPU, 3 seeds) | [ADD FINAL GPU RESULT] | [ADD FINAL GPU RESULT] | [ADD FINAL GPU RESULT] | [ADD FINAL GPU RESULT] | [ADD FINAL GPU RESULT] |

Report the GPU row as mean ± standard deviation across seeds 42, 1 and 2, taken from per_seed_test_macro_f1 in storage/models/m1/xlmr_lora_v1/model_registry.json.


*Table 7.15 — Per-category results*

| Category | Support (test) | Precision | Recall | F1 |
|---|---|---|---|---|
| SECTOR_SPECIFIC | 105 | [ADD FINAL GPU RESULT] |  |  |
| TAX_RATE_CHANGE | [ADD FINAL GPU RESULT] |  |  |  |
| IMPORT_EXPORT | [ADD FINAL GPU RESULT] |  |  |  |
| LABOUR_LAW | [ADD FINAL GPU RESULT] |  |  |  |
| BUSINESS_REGISTRATION | [ADD FINAL GPU RESULT] |  |  |  |
| PENALTY_ENFORCEMENT | [ADD FINAL GPU RESULT] |  |  |  |
| PRODUCT_STANDARD | [ADD FINAL GPU RESULT] |  |  |  |
| EPF_ETF_CHANGE | 0 | n/a | n/a | n/a |

EPF_ETF_CHANGE has zero support in the gold dataset and cannot be evaluated. It is reported as an explicit coverage limitation rather than dropped from the taxonomy, because silently removing a category would misrepresent what the deployed classifier can detect.


*Table 7.16 — Per-sector results*

| Sector | Support | Precision | Recall | F1 |
|---|---|---|---|---|
| grocery_retail | [ADD FINAL GPU RESULT] |  |  |  |
| food_service | [ADD FINAL GPU RESULT] |  |  |  |
| general_retail | [ADD FINAL GPU RESULT] |  |  |  |
...
|---|---|---|---|---|
| English | [ADD FINAL GPU RESULT] |  |  |  |
| Sinhala | [ADD FINAL GPU RESULT] |  |  |  |
| Tamil | [ADD FINAL GPU RESULT] |  |  |  |
| Short documents | [ADD FINAL GPU RESULT] |  |  |  |
| Long documents | [ADD FINAL GPU RESULT] |  |  |  |

Per-language slicing requires the primary_language normalisation of Section 7.2.1.9. If it has not been applied, the language rows must be marked "not evaluated" rather than filled with the overall figure.

| Insert the 8 x 8 confusion matrix exported by m1.model.eval from<br>storage/models/m1/eval_v1 here. |
|---|


*Figure 7.1 — Confusion matrix for the final classifier [ADD FINAL GPU RESULT]*
...
|---|---|---|---|---|
| ONNX FP32 | FP32 | [ADD FINAL GPU RESULT] |  |  |
| ONNX INT8 | INT8 | [ADD FINAL GPU RESULT] |  |  |

INT8 quantisation is adopted only if the accuracy cost against the FP32 artefact is below one percentage point of macro-F1; report both figures and state the decision.
...
|---|---|---|---|
| classifier_confidence | [ADD FINAL GPU RESULT] |  |  |
...
| Non-transformer baseline established | macro-F1 0.4980 (LogReg), 0.6167 (LinearSVC) | Achieved |
| Transformer pipeline validated end to end | CPU smoke test; registry written; gate_pass correctly false | Achieved |
| Transformer meets the RQ1 gate | [ADD FINAL GPU RESULT] | Pending GPU run |
| Extraction accuracy quantified per field, stage and language | Measurement engine, registry, report export | Framework achieved; figures pending |
| Trilingual explanation delivered | Schema, NLLB helper, review queue implemented | Partially achieved |
| Diffusion lag measured empirically | Watchers, matcher, lag views, preregistration | Instrument achieved; findings pending data |
| Platform integration across four modules | Shared verified store; contracts defined | Achieved for Module 1 side |
...
|---|---|
| [ADD FINAL GPU RESULT] test macro-F1 | storage/models/m1/xlmr_lora_v1/model_registry.json |
| [ADD FINAL GPU RESULT] per-category and per-sector F1 | storage/models/m1/eval_v1 |
| [ADD FINAL GPU RESULT] slice metrics | storage/models/m1/eval_v1 (requires primary_language normalisation) |
| [ADD FINAL GPU RESULT] confusion matrix and error analysis | storage/models/m1/eval_v1 |
| [ADD FINAL GPU RESULT] ONNX latency | Inference benchmark or worker logs |
