...
| Review-queue threshold                    | Derived; runtime is configuration-dependent                            | Decision margins on the V6 **validation** split separate cleanly: median 1.5954 for correct predictions against 0.3896 for errors. Candidate `M1_CLASSIFIER_MIN_MARGIN=0.40` flags 6.6% of rows and catches 55.6% of errors at 45.5% flag precision. The code default remains unset and truthfully reports `mode='disabled'`; `.env.example` opts into 0.40. The operating point is therefore not frozen until production review capacity and error yield are measured. Full sweep: `documentation/m1/analysis/MARGIN_THRESHOLD_ANALYSIS.md`.                                                                                                                                                                                                                                                          |
| scikit-learn version pin                  | **Pinned and active in the workspace**                                 | The frozen pipeline was fitted under scikit-learn **1.5.2**. The workspace `.venv` now reports Python 3.11.15, scikit-learn **1.5.2**, and joblib **1.5.3**, matching the `scikit-learn>=1.5.2,<1.6` serving/training pin. The earlier 1.8.0 mismatch and `uv sync` blocker are closed.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Sector prediction / V7 experiment         | Frozen primary unchanged; weighted V7 diagnostic completed and stopped | `linearsvc_v6_primary` still returns category only with `sectors: []`. After the rejected three-seed unweighted run, exactly one weighted seed-42 train/validation diagnostic ran on the no-leak 773/163 branch. It reached category macro-F1 `0.899862`, sector macro-F1 `0.884312`, exact-set `0.907975`, and partial-sector exact `4/9`. The test split was not loaded and no checkpoint was promoted. Do not continue to test/three seeds/export until a fresh temporal holdout and genuine EPF/ETF plus partial-sector evidence exist. |
| **Stage-E constrained summaries**         | **Backend slice shipped; deterministic audit complete; human/translation gates open** | The source-anchored non-LLM summariser and backfill/translation queue are implemented. A deterministic sample audit passed **80/80 English summaries**. Four false `summary_too_long` flags caused by `No.` sentence splitting were repaired, leaving seven genuine low-margin summaries for expert decisions. The SI/TA numeric-preservation audit failed **142/152** locale checks; 144 replacement jobs were queued but have not yet been drained by a translation worker. |
...
| Vault `works/` reorganization             | Completed                                                              | `PROGRAM_READINESS/log and works/` → `LOG_AND_WORKS/`; the active byte-identical `LOG_AND_WORKS/M1_PROGRAM_READINESS_MASTER_INDEX.md` copy was removed while the canonical `PROGRAM_READINESS/` file and older archived snapshot were retained; `_REORGANIZE_works.ps1` → `_tooling/`. Rules are in `00_WORKS_ORGANIZATION_INDEX.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **Documentation drift**                   | **Closed for this pass; recurrence risk remains**                      | The vault is canonical. On 2026-08-02 the numbered root series, final-work mirror, and frontend-generated documentation were refreshed one-way **from** the vault. The remaining gate is operational: keep the sync repeatable or add a drift check so the mirror cannot silently fork again. Evidence: `documentation/m1/structure_audit/DOCS_SYNC_REPORT.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Repository commit state                   | **Verified before this pass**                                          | The monorepo was on `main` with a clean working tree before the truth-ledger changes. The older note about a separate dirty `module-1` tree and untracked datasets no longer describes the current repository state.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| [x]    | [[00_INDEX#Document Index\|Document index]]                  | Lists docs 01–20 and states the correct root count: 20 numbered main docs + `00_INDEX.md` = 21 canonical Markdown files. |
...
| [~]    | [[06_M1_Training_Evaluation#10. Backfill and the Pre-Viva Checklist\|Backfill and pre-viva checks]] | The first live classification run has happened, closing the previous blocker. **Remaining gate:** preserve a reproducible run summary (row counts, backend/model name, margin-mode distribution, failures/retries) and complete the pre-viva sanity checks rather than treating “ran once” as full backfill evidence. |

## 07 · Deployment & Integration — [[07_M1_Deployment_Integration]]

**Parent task:** [~] Serve the trained classifier and integrate it into backend workflows. **The serving path changed shape:** the frozen model is an in-process joblib pipeline, not an ONNX artifact on Fly.io. Backend integration, migration, first live classification, and the classifier-review UI are done. Remaining work is operational acceptance: latency/canary evidence, the margin operating-point decision, and margin-aware monitoring.
...
| [x] | [[07_M1_Deployment_Integration#4. API Integration\|API integration]] | Backend and consumer are connected: `classify_gazette` persists `decision_margin` + `model_name`; the review-queue endpoint reports its signal/mode; `/admin/m1/pipeline/classifier-review` handles `mode='disabled'`, configured thresholds, margin ranking, and `classifier_confidence: null` without displaying a false percentage. |
...
| [~] | [[07_M1_Deployment_Integration#9. Validation and Acceptance Criteria\|Acceptance criteria]] | Reproduction parity, artifact hashes, compatible scikit-learn 1.5.2 runtime, first live classification, and UI contract handling are proven. **Gates remaining:** record service latency/canary-style health evidence and choose/validate the production review-margin operating point. |
...
| [~] | [[09_M1_Annotation_Guidelines#9. SME Awareness Survey Instrument\|SME survey instrument]] | Q1–Q8, the authenticated engine, and public `/portal/m1/survey` route now exist. EN/SI/TA recruitment copy, consent, safe auth return paths, and recruitment-channel tracking were browser-verified. **Remaining gate:** real fieldwork is still 0/≥100 unique SME completions. |
...
| [x] | [[11_M1_API_Reference#4. Classification and Verification\|Classification and verification]] | The nullable-confidence contract is implemented and consumed. Responses carry `confidence_type: "not_available_uncalibrated_linearsvc"` plus `decision_score`, `decision_margin`, `second_category`, and `class_scores`; the classifier-review UI ranks margins and does not render them as percentages. |
...
| [x] | [[14_M1_Tracking_Workflows#3. A2 — Admin Review-Queue Triage\|A2 review queue]] | Implemented at `/admin/m1/pipeline/classifier-review`: consumes the review endpoint, handles `mode='disabled'` and configured thresholds, ranks by decision margin, and tolerates `classifier_confidence: null` without presenting a probability. |
...
| [~] | [[15_M1_Folder_Reference#2. `enigmatrix-ml/` — the ML Monorepo\|ML monorepo guide]] | Samplers, preprocessing, evaluation scaffolds, frozen LinearSVC, rejected LoRA/V7 lines, and the NLLB translation path are documented by use. Stage-E summarisation is now correctly located in the backend; remaining ML guide work is refresh/acceptance detail, not a missing summariser. |
...
| [~] | [[16_M1_Development_Roadmap#Phase 3 — Annotation + classification (BUILD_07 §C–D + BUILD_11)\|Phase 3 annotation and classification]] | **Model selection, integration, and margin/category monitoring are implemented.** Live evidence is 898 classified rows, 18 below the current 0.40 margin, and zero confirmed/corrected review outcomes. Therefore `M1_CLASSIFIER_MIN_MARGIN=0.40` remains explicitly **provisional**, not frozen. Next: obtain expert outcomes for the seven active review rows, measure correction/override yield, then freeze or revise the threshold; separately collect genuine `EPF_ETF_CHANGE` evidence. |
...
| [~] | [[16_M1_Development_Roadmap#Phase 5 — Research findings + survey deployment\|Phase 5 findings and survey]] | Sequence survey deployment, F1–F6 notebooks, retraining cadence, and rollback. `/portal/m1/survey` is now built and browser-verified with EN/SI/TA, consent, safe auth return paths, and recruitment-channel capture. **The critical path is real fieldwork:** 0 of ≥100 unique SME completions. Plan: `PHASE5_RESEARCH_FINDINGS/PHASE5_GAP_CLOSURE_PLAN.md`. |
...
| [~] | [[17_M1_Repo_Structure_Map#6. The two copies of this documentation set\|Doc-set fork]] | The vault is canonical; `enigmatrix-docs/m1/` is refreshed *from* it, never merged back. The root series/final mirror were refreshed again on 2026-08-02 and frontend docs regenerated. **Gate:** add an automated/scheduled drift check; a correct manual sync still permits future recurrence. |
...
| Status | Linked subtask | Task breakdown and downstream use |
|---|---|---|
| [x] | [[18_M1_Dataset_And_Model_Lineage#1. Lineage at a glance\|Lineage chain]] | V4 → V5 → frozen V6, then a separate V7 working branch. The reporting split stayed 777/166/167 through V6; the V7 no-leak experiment explicitly removed seven records and used 773/163/167 without changing the gold history. |
...
| [x] | [[18_M1_Dataset_And_Model_Lineage#3. Models trained on these datasets\|Model table]] | Production/baseline models plus the rejected 1,103-row V7 multitask experiment, with dataset, metrics, and verdict. The evidence base for why neither transformer line was promoted. |
...
| [~] | [[18_M1_Dataset_And_Model_Lineage#5. Standing constraints\|Standing constraints]] | The V6 test split is spent (including the rejected V7 run); `EPF_ETF_CHANGE` needs documents not resampling; artifact paths and label orders are contracts; tables must be re-derived. **Gate:** targeted collection and a fresh temporal holdout are required before new model claims. |
...
| [~] | [[19_M1_Regulation_Summarization#7. Evaluation Without Reference Summaries\|Evaluation protocol]] | The deterministic 80-row English audit passed 80/80 grounding/source-hash/hard-flag checks. The separate planned human harm/faithfulness review and E vs E′ verifier ablation remain open; an engineering audit is not relabelled as human evidence. |
| [~] | [[19_M1_Regulation_Summarization#11. Validation and Acceptance Criteria\|Acceptance criteria]] | Four false `summary_too_long` reviews were repaired, leaving 7 genuine classifier-review rows. The SI/TA numeric audit passed only 10/152 checks; 144 manual-priority replacement jobs were queued and remain pending because no usable translation worker is online. **Gate:** drain/re-audit translations, obtain human quality review, and complete full-row coverage. |

## 20 · Multitask Classifier Upgrade (V7) — [[20_M1_Multitask_Classifier_Upgrade]]

**Parent task:** [~] Add an experimental V7 classifier line that can emit category, affected study sectors, and derived SME relevance without modifying the frozen `linearsvc_v6_primary`. Steps 41-47 and the first Step-48 weighted recovery diagnostic are executed in Kaggle. The validation-only seed-42 run recovered to category macro-F1 `0.899862` and sector macro-F1 `0.884312`, but partial-sector exact match was `4/9` and the category gate is `0.92`. The line remains non-promotable; no test, three-seed recovery, threshold freeze, or export followed.

| Status | Linked subtask | Task breakdown and downstream use |
|---|---|---|
| [x] | [[20_M1_Multitask_Classifier_Upgrade#1. Step 41 — the V6 audit, and what it changed\|Step 41 V6 audit]] | Read-only Kaggle audit over the source V6 parquets. Columns: `key`, `text`, `category`, `sectors`, `language`, `date`; no explicit `is_sme_relevant`, so relevance is derived from sectors for the working branch. Source split sizes: `777/166/167 = 1110`; unknown categories/sectors: none; duplicate keys: none. Exact-text leakage was found: 8 within-split duplicate-text rows and 6 train-val overlap rows. Reports: `/kaggle/working/storage/reports/m1/v6_multitask_audit`. |
| [x] | [[20_M1_Multitask_Classifier_Upgrade#2. Step 42 — the V7 dataset\|Step 42 no-leak training dataset]] | Built derived working dataset `/kaggle/working/storage/datasets/m1/m1_regulations_v6_1110_multitask_noleak`; source unchanged. Rows `1110 -> 1103`; split sizes `train=773`, `val=163`, `test=167`; removed 7 exact-text duplicates; zero key/text overlap after cleanup. This is a Kaggle working training artifact with the original six columns, not yet a formal enriched V7 release package with stored `sector_vector`/`relevance_label` columns. |
| [x] | [[20_M1_Multitask_Classifier_Upgrade#2. Step 42 — the V7 dataset\|Step 43 clean audit]] | Re-audited the cleaned dataset under `/kaggle/working/storage/reports/m1/v6_multitask_noleak_audit`; consistency errors `0`, text overlap `0`, duplicate key overlap `0`, unknown labels `0`. Clean distribution: 805 irrelevant / 298 relevant by derived sector rule. |
| [x] | [[20_M1_Multitask_Classifier_Upgrade#3. Steps 43–44 — label contract and architecture\|Steps 44 + 46 parser/trainer patch]] | Patched the Kaggle working repo so `labels.py` handles pyarrow `numpy.ndarray` sector lists correctly, and `train_xlmr.py` emits category accuracy/macro-F1/weighted-F1, sector micro/macro/exact-set metrics, derived relevance accuracy, per-sector diagnostics, and saved registry/model/labels/tokenizer artifacts. |
| [x] | [[20_M1_Multitask_Classifier_Upgrade#4. Steps 45–49 — training protocol\|Step 45 smoke]] | One-epoch seed-42 smoke completed at `/kaggle/working/storage/models/m1/xlmr_multitask_v6_noleak_metrics_smoke_seed42`. It proved the cleaned path, patched parser, metric registry, model save, labels, and tokenizer export; metrics were intentionally not promotion evidence. |
...
| 06 | Backfill / pre-viva | First live run happened, but a reproducible run summary and full sanity checklist are not recorded | Capture counts/backend/margin modes/failures and finish the pre-viva checks |
| 07 | ONNX export | No transformer met the gate | Nothing — deliberately not needed |
| 07 | Acceptance | Runtime compatibility and first live/UI flow are proven; latency/canary and operating-threshold evidence are not | Record service evidence and validate the production margin threshold |
| 08 | DB / API / routes / task graph / DoD | First classification/summary flow has live evidence, but Phase-4 automation and whole-system DoD remain incomplete | Phase 4 activation + end-to-end evidence pack |
| 09 | SME survey instrument | Public `/portal/m1/survey` now exists; field responses do not | Recruit and obtain ≥100 completed responses from unique SMEs |
...
| 12 | Classifier monitoring | Margin and category-distribution monitoring are implemented; 0.40 has no review-yield evidence | Complete expert reviews, record confirmed/corrected outcomes, then freeze or revise the threshold |
...
| 19 | Evaluation | Deterministic 80-English audit is complete; SI/TA numeric audit failed 142/152 and replacement translations are pending; no human evaluation exists | Drain and re-audit SI/TA jobs, then run the 80-document human eval + E vs E′ ablation |
| 20 | Weighted-loss recovery | Seed-42 validation-only weighted diagnostic recovered category macro-F1 to `0.899862` and sector macro-F1 to `0.884312`, but partial exact is `4/9` and category remains below `0.92` | Collect fresh temporal, EPF/ETF, and partial-sector evidence before any final run or claim |
...
| Multitask upgrade specified and executed through the first Step-48 diagnostic | [x] Doc 20 synced — the rejected unweighted history remains, and the weighted seed-42 validation-only recovery is recorded. Verdict: recovery is real but still not promotable; stop before test/three-seed/export pending new evidence. |
...
| Model & training | **05**, **18** | 06, **20** *(weighted seed-42 validation diagnostic completed; new-evidence gate open)* | — |
