...
2. Watch specifically: **SI/TA slice F1** (if >5 pp below EN, the Wijesekara-era text quality is leaking into labels — cross-check rows extracted pre-Session-69 against the font-aware backfill in [[PHASE2_TRILINGUAL_AUTOCHAIN_PLAN]]), and **extraction_method slice** (tesseract-extracted docs materially worse → OCR CER is the binding constraint, feeds the Surya decision).
3. Also run the chunk-contract A/B from [[PHASE2_CHUNK_CONTRACT_PLAN]] (cleaned_text-head vs classification_chunk) — decide the inference input on evidence before the artifact ships.
